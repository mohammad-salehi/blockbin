...
export default function Header({
  ...
  // تشخیص دسکتاپ/موبایل با matchMedia (پایدارتر از innerWidth)
  useEffect(() => {
    ...
    setFromMQ();
    if (mq.addEventListener) mq.addEventListener('change', setFromMQ);
    else mq.addListener(setFromMQ);

    return () => {
      if (mq.removeEventListener) mq.removeEventListener('change', setFromMQ);
      else mq.removeListener(setFromMQ);
    };
  }, []);
  ...
  const NavItem = ({ item, variant }) => {
    const isActive = item.link === pathName;

    const base =
      variant === 'desktop'
        ? 'flex items-center px-4 py-2 rounded-md transition select-none'
        : 'flex items-center px-4 py-2 rounded-md transition w-full justify-start select-none';

    const activeCls = 'bg-bgPrimary text-primary';
    const inactiveCls = 'hover:bg-bgPrimary text-textColor';
    ...
    return (
      <Link
        ...
        onClick={() => {
          if (variant === 'mobile') setIsMobileOpen(false);
        }}
      >
        <span className={`${base} ${isActive ? activeCls : inactiveCls}`}>
          <span className={`text-xl ml-2 ${isActive ? 'text-primary' : 'text-textColor'}`}>
            {item.icon}
          </span>
          <span className={isActive ? 'text-primary' : 'text-textColor'}>{item.label}</span>
        </span>
      </Link>
    );
  };
  ...
  return (
    <>
      ...
      {isMobileOpen && (
        <div className="fixed inset-0 z-40 bg-gray-500/50 md:hidden" onClick={closeSidebar} />
      )}

      <header
        className={`relative z-50 w-full bg-boxColor dark:bg-boxColor-dark flex items-stretch justify-between ${
          isOpen ? 'rounded-bl-md rounded-br-md' : ''
        } shadow-sm px-2 sm:px-6`}
      >
        <div className="flex items-center gap-5 text-textColor min-w-0">
          {/* دکمه منو موبایل */}
          <button
            type="button"
          ...
          {/* دکمه منو دسکتاپ وقتی سایدبار بسته است */}
          <button
            type="button"
            className={`${!isOpen ? 'hidden md:flex' : 'hidden'} items-center justify-center h-9 w-9 rounded-full hover:bg-bgPrimary cursor-pointer`}
            onClick={handleMenuClick}
            aria-label="sidebar"
          >
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          {/* منوی دسکتاپ: همیشه در DOM هست، فقط با کلاس مخفی/نمایان میشه */}
          <nav
            className={`items-center p-2 mt-0 gap-x-2 min-w-0 overflow-x-auto whitespace-nowrap hidden md:flex ${
              isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none w-0 overflow-hidden'
            }`}
            dir="rtl"
          >
            {navItems.map((item) => (
              <NavItem key={item.link} item={item} variant="desktop" />
            ))}
          </nav>
        </div>
      ...
      {isMobileOpen && (
        ...
        >
          <div className="flex flex-col gap-2">
            {navItems.map((item) => (
              <NavItem key={item.link} item={item} variant="mobile" />
            ))}
          </div>
        </nav>
      )}
