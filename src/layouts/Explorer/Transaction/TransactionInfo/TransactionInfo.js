...
const TransactionInfo = ({ TotalUSDValue, SetTotalUSDValue }) => {
    ...
    return (
        // ✅ بوردر بیرونی دست نخورده
        <div
            className='bg-gradient-main-2 border border-boxBorderColor rounded-xl main-animated-border-box'
            style={{ "--dynamic-color": `${Networks.find(item => item.symbole === network).color}` }}
        >
            <div className="flex flex-col xl:flex-row xl:justify-between xl:items-center border-b border-b-boxBorderColor p-4 gap-3">
                <div className="flex items-center gap-3">
                    <div className="h-10 w-10 rounded-xl bg-bgColor border border-boxBorderColor flex items-center justify-center overflow-hidden shadow-sm">
                        <img src={`/images/${network}.png`} className="w-8 h-8 object-contain" alt={network} />
                    </div>

                    <div className="leading-tight">
                        <h6 className="text-lg xl:text-xl font-semibold text-textColor">
                ...
                <div className="flex flex-wrap items-center gap-2 xl:gap-3 text-primary">
                    {(Label === null || Label.length === 0) ? (
                        <button
                            type="button"
                            className="flex items-center justify-center border bg-bgColor text-textColor border-boxBorderColor transition hover:opacity-90 h-9 w-9 rounded-full cursor-pointer"
                            onClick={() => setIsOpen(true)}
                            aria-label="افزودن برچسب"
                            title="افزودن برچسب"
                        >
                            <svg width="22" height="22" viewBox="0 0 50 50" fill="currentColor" aria-hidden="true">
      ...
      L28.181,38.755z" />
                        ...
                        <button
                            type="button"
                            className="flex items-center gap-2 bg-orange-300/90 text-black transition hover:bg-orange-300 h-9 rounded-4xl cursor-pointer px-4 shadow-sm"
                            onClick={() => {
                                deleteLabel()
                            }}
                            title="حذف برچسب"
                        >
                            <span className="text-sm font-medium">{Label[0].label}</span>
                            <span className="text-[11px] opacity-70">(حذف)</span>
                        </button>
                    )}
                    ...
                    <div className="px-3 py-1.5 rounded-full bg-bgColor border border-boxBorderColor text-textColor text-sm">
                    ...
                    <div>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 text-textColor p-4">
                            ...
                            <div className="rounded-xl bg-bgColor/40 border border-boxBorderColor p-4 shadow-sm">
                                ...
                                <p className="text-base font-semibold flex items-center gap-2 tabular-nums">
                                    <svg fill="currentColor" height="20" width="20" className='inline-block' version="1.1" id="Filled_Icons"
                                    ...
                                    <span className={`${Owner !== null ? 'text-primary' : 'text-textColor'} `}>
                                        {TrValue !== null ? (
                                            <span>
                                                {(TrValue).toLocaleString()}<small className='ml-1 opacity-80'>{network}</small>
                                            </span>
                                        ) : 'نامشخص'}
                                    </span>
                                </p>
                            ...
                            <div className="rounded-xl bg-bgColor/40 border border-boxBorderColor p-4 shadow-sm">
                                ...
                                <p className="text-base font-semibold flex items-center gap-2 tabular-nums">
                                    <svg width="20" height="20" className='inline-block' viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    {Fee !== null ? (
                                        <span>
                                            {(Fee).toLocaleString()}<small className='ml-1 opacity-80'>{network}</small>
                                        </span>
                                    ) : 'نامشخص'}
                                </p>
                            </div>
                            ...
                            <div className="rounded-xl bg-bgColor/40 border border-boxBorderColor p-4 shadow-sm">
                                ...
                                <p className="text-base font-semibold flex items-start gap-2">
                                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className='inline-block mt-0.5'>
                                        <path d="M7 10H17M7 14H12M7 3V5M17 3V5M6.2 21H17.8C18.9201 21 19.4802 21 19.908 20.782C20.2843 20.5903 20.5903 20.2843 20.782 19.908C21 19.4802 21 18.9201 21 17.8V8.2C21 7.07989 21 6.51984 20.782 6.09202C20.5903 5.71569 20.2843 5.40973 19.908 5.21799C19.4802 5 18.9201 5 17.8 5H6.2C5.0799 5 4.51984 5 4.09202 5.21799C3.71569 5.40973 3.40973 5.71569 3.21799 6.09202C3 6.51984 3 7.07989 3 8.2V17.8C3 18.9201 3 19.4802 3.21799 19.908C3.40973 20.2843 3.71569 20.5903 4.09202 20.782C4.51984 21 5.07989 21 6.2 21Z" stroke="currentColor" />
                                    </svg>
                                    <span className="leading-6">
                                        {Time !== null ? (
                                            <span style={{ margin: "0px" }}>
                                                {(GetMyTime(Time).hour + ':' + GetMyTime(Time).minute + ' - ' + moment(GetMyTime(Time).year + '-' + GetMyTime(Time).month + '-' + GetMyTime(Time).day, 'YYYY/MM/DD').locale('fa').format('YYYY/MM/DD'))} ({timeSince(Time)})
                                            </span>
                                        ) : 'نامشخص'}
                                    </span>
                                </p>
                            ...
                            <div className='flex items-center w-full'>
                                <button
                                    className='border border-primary rounded-lg text-primary w-full py-2 cursor-pointer transition hover:bg-primary/10 active:scale-[0.99]'
                                    onClick={() => {
                            ...
                            <div className='flex items-center w-full'>
                                <button
                                    className='border border-primary rounded-lg bg-primary text-white w-full py-2 cursor-pointer transition hover:opacity-90 active:scale-[0.99] shadow-sm'
            ...
            <Modal open={isOpen} onClose={() => { setIsOpen(false) }}>
                ...
                <div className="fixed inset-0 flex z-50 backdrop-blur-sm bg-white/10">
                    <Modal.Panel className="w-full max-w-xl rounded-2xl bg-boxColor shadow-lg mt-50 text-textColor p-5 border border-boxBorderColor">
                        ...
                        <button
                            className='bg-primary border border-primary rounded-lg text-white w-full py-2 cursor-pointer mt-4 transition hover:opacity-90 active:scale-[0.99] shadow-sm'
                            onClick={() => AddLabel()}
                        >
                            ثبت
                        </button>
