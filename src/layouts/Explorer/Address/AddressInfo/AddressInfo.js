    ...
    GetRequest(`${serverAddress}/explorer/address-detail?query=${hash}`)
      .then((response) => {
        ...
        if (response.status === 200) {
          SetOwner(response.data.data.address_detail.entity.name ? response.data.data.address_detail.entity : null)
          ...
          SetIdentificationBy(response.data.data.address_detail.address_label.length !== 0 ? response.data.data.address_detail.address_label[0] : null)
        }
        SetLoading1(false)
      })
      .catch((err) => {
    ...
    >
              ...
              {(Tag === null || Tag.length === 0) ? (
                <button
                  ...
                  className="group inline-flex items-center justify-center h-10 w-10 rounded-2xl
                                 ...
                                 hover:bg-bgColor transition active:scale-[0.98] shadow-sm"
                ...
                <div className="group rounded-2xl bg-bgColor/50 border border-boxBorderColor p-4 shadow-sm backdrop-blur hover:bg-bgColor/70 hover:shadow-md transition">
                  ...
                  <p className="text-base font-extrabold flex items-center gap-2 tabular-nums">
                    <span className="h-9 w-9 rounded-2xl grid place-items-center bg-bgColor border border-boxBorderColor">
                      <svg
                        fill={Risk === null ? "currentColor" : Risk < 25 ? "green" : Risk < 50 ? "blue" : Risk < 70 ? "orange" : "red"}
                        height="18"
                        width="18"
                        viewBox="0 0 512 512"
                      >
      ...
      C281.638,399.11,270.06,411.018,256.498,411.018z"/>
                ...
                <div className="group rounded-2xl bg-bgColor/50 border border-boxBorderColor p-4 shadow-sm backdrop-blur hover:bg-bgColor/70 hover:shadow-md transition">
                  ...
                  <p className="text-base font-extrabold flex items-center gap-2">
                    <span className="h-9 w-9 rounded-2xl grid place-items-center bg-bgColor border border-boxBorderColor">
                      <svg fill="currentColor" width="18" height="18" viewBox="0 0 97 97">
                        <path d="M95,44.312h-7.518C85.54,26.094,70.906,11.46,52.688,9.517V2c0-1.104-0.896-2-2-2h-4.376c-1.104,0-2,0.896-2,2v7.517
      ...
      C70.906,85.54,85.54,70.906,87.482,52.688H95c1.104,0,2-0.896,2-2v-4.376C97,45.207,96.104,44.312,95,44.312z"/>
                    ...
                    <span>{IdentificationBy !== null ? IdentificationBy : "نامشخص"}</span>
                  </p>
                </div>
                ...
                <div className="group rounded-2xl bg-bgColor/50 border border-boxBorderColor p-4 shadow-sm backdrop-blur hover:bg-bgColor/70 hover:shadow-md transition">
                  ...
                  <p className="text-base font-extrabold flex items-center gap-2">
                    <span className="h-9 w-9 rounded-2xl grid place-items-center bg-bgColor border border-boxBorderColor">
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
                        <path d="M19 7.24997H18.75V4.99997C18.7474 4.53665 18.5622 4.09305 18.2345 3.76543C17.9069 3.43781 17.4633 3.25259 17 3.24997
      ...
      C20.7474 8.53665 20.5622 8.09305 20.2345 7.76543C19.9069 7.43781 19.4633 7.25259 19 7.24997Z"
                        ...
                        <path d="M16.5 15.25C17.1904 15.25 17.75 14.6904 17.75 14C17.75 13.3096 17.1904 12.75 16.5 12.75C15.8096 12.75 15.25 13.3096 15.25 14C15.25 14.6904 15.8096 15.25 16.5 15.25Z"
                          fill="currentColor" />
                      </svg>
                    </span>
                    <span>{metadata !== null ? metadata : "نامشخص"}</span>
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4 h-full">
                <button
                  className=" rounded-2xl bg-primary text-white shadow-sm
                                 hover:opacity-95 transition active:scale-[0.99] flex items-center justify-center gap-2 h-full cursor-pointer"
                ...
                <button
                  className="h-11 rounded-2xl border border-primary text-primary bg-bgColor/40 backdrop-blur
                                 hover:bg-primary/10 transition active:scale-[0.99] flex items-center justify-center gap-2 cursor-pointer"
                ...
                >
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" className="opacity-95">
                    ...
                    <path d="M3.15 7V17.353C3.14925 17.9813 3.39203 18.5886 3.83074 19.0397C4.27008 19.4914 4.87049 19.7492 5.50099 19.75H17.899
          ...
          <Modal open={isFolderOpen} onClose={() => setFolderIsOpen(false)}>
            ...
            <div className="fixed inset-0 flex z-50 backdrop-blur-sm bg-black/30">
              <Modal.Panel className="w-full max-w-xl rounded-3xl bg-boxColor shadow-2xl mt-50 text-textColor p-6 border border-boxBorderColor">
                ...
                <p className="text-xs text-textTitleColor mt-1">یک پرونده را انتخاب کنید تا این آدرس به آن اضافه شود.</p>
